...
mkdir build
cd build
"$INSTALLDIR/bin/qt-configure-module" .. -- -DCMAKE_PREFIX_PATH="$INSTALLDIR" -DQT_GENERATE_SBOM=ON
cmake --build . --parallel
ninja install
cd ../../
rm -fr "qtshadertools-everywhere-src-$QT"
...
mkdir build
cd build
"$INSTALLDIR/bin/qt-configure-module" .. -- -DCMAKE_PREFIX_PATH="$INSTALLDIR" -DQT_GENERATE_SBOM=ON -DFEATURE_wayland_server=OFF
cmake --build . --parallel
ninja install
cd ../../
rm -fr "qtdeclarative-everywhere-src-$QT"
