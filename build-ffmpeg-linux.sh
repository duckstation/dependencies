...
d3c12eb324edf05e214608b6a395a51dd95770ed9d45520185d6c3a206811c99  AMF-headers-v$AMF.tar.gz
...
cmake -B build-ds -G Ninja -DCMAKE_INSTALL_PREFIX="$DEPSINSTALLDIR" -DCMAKE_PREFIX_PATH="$DEPSINSTALLDIR" -DCMAKE_BUILD_TYPE=Release \
       -DCMAKE_POSITION_INDEPENDENT_CODE=ON -DSHADERC_ENABLE_HLSL=OFF -DSHADERC_SKIP_EXAMPLES=ON -DSHADERC_SKIP_TESTS=ON -DSPIRV_SKIP_TESTS=ON
cmake --build build-ds --parallel
cmake --install build-ds
cd ..
